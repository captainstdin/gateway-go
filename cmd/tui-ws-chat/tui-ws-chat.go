...
func (m *model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case wsMsg:
		raw := string(msg)
		// Parse the welcome message to extract our client ID
		if m.myClientID == "" && strings.HasPrefix(raw, "Welcome! Your ID are: ") {
			m.myClientID = strings.TrimPrefix(raw, "Welcome! Your ID are: ")
			m.appendMsg("✓", colorGreen, fmt.Sprintf("My ID: %s", m.myClientID))
			return m, readCmd(m.conn)
		}
		// Parse chat messages in "clientID:message" format
		if idx := strings.Index(raw, ":"); idx > 0 {
			senderID := raw[:idx]
			content := raw[idx+1:]
			if senderID == m.myClientID {
				m.appendMsg("Me:", colorGreen, content)
			} else {
				m.appendMsg(senderID+":", colorCyan, content)
			}
		} else {
			m.appendMsg("<", colorCyan, raw)
		}
		return m, readCmd(m.conn)
